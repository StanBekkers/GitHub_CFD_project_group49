...
velocities = [0.05, 0.20, 0.50];  % Inlet velocities [m/s]
wattages = [300];            % Heat dissipation power [W]

% Pre-allocate arrays for compiling the final summary table
num_cases = length(fin_types) * length(velocities) * length(wattages);
summary_fin = cell(num_cases, 1);
summary_vel = zeros(num_cases, 1);
summary_wat = zeros(num_cases, 1);
summary_nu  = zeros(num_cases, 1);
summary_dp  = zeros(num_cases, 1);
case_counter = 1;
...
% Active footprint dimensions
A_core   = ((1 - 2*l_base_frac) * XMAX) * 0.015;     % m² active area

% =========================================================================
% %% MAIN AUTOMATED PARAMETER SWEEP LOOP (3-WAY SWEEP)
% =========================================================================
...
for f_idx = 1:length(fin_types)
    ...
    for v_idx = 1:length(velocities)
        ...
        for w_idx = 1:length(wattages)
            P_core = wattages(w_idx);
            
            fprintf('\n=======================================================================\n');
            fprintf('  LAUNCHING SOLVER | Fin: %s | Velocity: %0.2f m/s | Power: %g W\n', upper(fin_type), U_IN, P_core);
            fprintf('=======================================================================\n');
            
            % 1. Re-initialize memory arrays for this specific run
            init(); 
            
            % Recalculate physical heat zone properties based on current P_core loop variable
            q_flux_core = P_core / A_core;
            R_core = t_copper / (k_copper * A_core);
            
            heat_zone = struct('x_start', l_base_frac*XMAX,    'x_end', (1-l_base_frac)*XMAX, ...
            ...
            elseif strcmp(fin_type, 'nofins')
                ...
                error('Invalid fin_type specified.');
            end
            cooler_layout = layout_wall | layout_fins;

            % 3. Generate Baseline (No Flow) Conduction Profile once per geometry/wattage combination
            if v_idx == 1
                T_mock = 293.15 * ones(NPI+2, NPJ+2);
                dT_max = 3.35 * (P_core / 450); % Scales temperature dynamically with the wattage loop
                for I_cell = 1:NPI+2
                    for J_cell = J_fluid_bottom:J_fluid_top
                        dx = x(I_cell) - 0.5 * XMAX;
                        dy = y(J_cell) - 0.5 * YMAX;
                        T_mock(I_cell, J_cell) = 293.15 + dT_max * exp(-(dx^2 / (2 * 0.028^2) + dy^2 / (2 * 0.010^2)));
                    end
                end
            ...
            while (iter <= MAX_ITER && (SMAX > SMAXneeded || SAVG > SAVGneeded))
                ...
                if mod(iter, NPRINT) == 0
                    I_chk = round((NPI+1)/2);
                    J_chk = round((NPJ+1)/2);
                    du = d_u(I_chk,J_chk)*(pc(I_chk-1,J_chk) - pc(I_chk,J_chk));
                    dv = d_v(I_chk,J_chk)*(pc(I_chk,J_chk-1) - pc(I_chk,J_chk));
                    fprintf ('Iter %4d:\t du/u=%10.2e\t dv/v=%10.2e\t SMAX=%10.2e\t SAVG=%10.2e\n', iter, du/u(I_chk,J_chk), dv/v(I_chk,J_chk), SMAX, SAVG);
                end
                iter = iter + 1;   
            end

            % --- POST-PROCESSING: THERMODYNAMIC OVERALL NUSSELT NUMBER ---
            fprintf('  Calculating Overall Nusselt number...\n');
            
            k_fluid = 0.6;                                  % Molecular water conductivity [W/m*K]
            H_channel = YMAX * (1 - 2 * h_base_frac);       % Channel height [m]
            Dh = 2 * H_channel;                             % Hydraulic diameter [m]

            % Find the peak hotspot temperature in the domain
            T_chip_max = max(max(T));
            T_in = 293.15;                                  % Inlet temperature [K]
            
            % Heat transfer area based on the chip length in the 2D plane (scaled by physical depth t_fluid)
            L_chip = (1 - 2*l_base_frac) * XMAX;            % Active chip length [m]
            A_transfer = L_chip * t_fluid;                  % Heat transfer area [m^2]
            
            % Overall heat transfer coefficient [W/m^2*K]
            h_avg = P_core / (A_transfer * (T_chip_max - T_in) + 1e-12);
            
            % Overall Nusselt Number
            Nu_avg = h_avg * Dh / k_fluid;
            fprintf('  Overall Nusselt Number: %0.2f\n', Nu_avg);
            
            % Store results in compiling arrays for final summary table
            p_inlet_avg = mean(p(2, J_fluid_bottom:J_fluid_top));
            
            summary_fin{case_counter} = upper(fin_type);
            summary_vel(case_counter) = U_IN;
            summary_wat(case_counter) = P_core;
            summary_nu(case_counter)  = Nu_avg;
            summary_dp(case_counter)  = p_inlet_avg;
            ...
            fprintf('  Run completed successfully! Saved figures and .dat files for: %s @ %0.2f m/s (%g W)\n', upper(fin_type), U_IN, P_core);
        end % w_idx loop
    end % v_idx loop
end % f_idx loop

% =========================================================================
% %% COMPILING AND EXPORTING FINAL SUMMARY TABLE
% =========================================================================

summary_table = table(summary_fin, summary_vel, summary_wat, summary_nu, summary_dp, ...
    'VariableNames', {'Fin_Type', 'Velocity_m_s', 'Wattage_W', 'Avg_Nusselt', 'Pressure_Drop_Pa'});

disp(' ');
disp('=======================================================================');
disp('                        FINAL PARAMETER SWEEP SUMMARY                  ');
disp('=======================================================================');
disp(summary_table);
disp('=======================================================================');

% Save compiled table directly as a CSV file for post-processing
csv_filename = fullfile(results_dir, 'Parameter_Sweep_Summary.csv');
writetable(summary_table, csv_filename);
fprintf('  Saved final summary table to: %s\n', csv_filename);
